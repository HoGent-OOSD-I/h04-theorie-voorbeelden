...
public class Staafdiagram {
    ...
    private void toonStaafdiagram() {
        ...
        IO.println("Verdeling examenresultaten:");

        for (int index = 0; index < verdeling.length; index++) { // <.>
            if (index == verdeling.length - 1) // <.>
                IO.print(String.format("%5d: ", index * 10));
            else
                IO.print(String.format("%02d-%02d: ", index * 10, index * 10 + 9));

            IO.println("*".repeat(verdeling[index])); // <.>
        }
    }
}
